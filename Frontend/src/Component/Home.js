...
import { Carousel } from 'react-responsive-carousel';
import 'react-responsive-carousel/lib/styles/carousel.min.css';
import { FaWhatsapp, FaArrowRight, FaAngleRight, FaChevronRight } from 'react-icons/fa';
...
const Home = () => {
  ...
  // Chunk Logic for Process Carousel
  const chunkSize = isMobile ? 1 : 3;
  const processChunks = [];
  for (let i = 0; i < processData.length; i += chunkSize) {
    processChunks.push(processData.slice(i, i + chunkSize));
    ...
    <Box sx={{ width: '100%' }}>
      ...
      <Box sx={{ py: { xs: 4, md: 10 }, px: { xs: 1, md: 8 }, backgroundColor: '#f9f9f9', overflow: 'hidden' }}>
        <Container maxWidth="xl" sx={{ px: { xs: 1, md: 3 } }}> 
            ...
            {/* ================= Our Products ================= */}
      <Box sx={{ py: { xs: 4, md: 10 }, backgroundColor: "#ffffff" }}>
        <Container maxWidth="xl">
          <Typography
            variant="h3"
            fontWeight={900}
          ...
          >
            {product.slice(0, 4).map((prod, index) => {
              const hideOnDesktop = index === 3; // hide 4th on desktop

              return (
                <Grid
                  item
                  xs={6}        // mobile: 2 per row → 4 total
            ...
            >
              <Button
                ...
                sx={{
                  ...
                  "&:hover": { boxShadow: "0 12px 30px rgba(0,0,0,0.3)" },
                }}
              >
      ...
      <Box sx={{ py: { xs: 4, md: 10 }, px: { xs: 1, md: 8 }, background: '#f4f6f8', overflow: 'hidden' }}>
        <Container maxWidth="xl">
            ...
            <Box sx={{ maxWidth: '100%', mx: 'auto', pb: 4 }}>
              ...
              >
                {processChunks.map((chunk, i) => (
                  <Box key={i} sx={{ pb: 6, px: { xs: 1, md: 2 } }}> 
                    <Grid container spacing={3} justifyContent="center">
                      {chunk.map((proc, index) => (
                        <Grid item xs={12} md={4} key={index}>
                          <Paper
